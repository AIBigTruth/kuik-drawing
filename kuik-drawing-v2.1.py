...
from PyQt5.QtCore import Qt, QPoint, QRect, pyqtSignal, QTimer, QEvent
...
class DrawingApp(QMainWindow):
    def __init__(self):
        ...
        # 清空画布双击检测
        self.clear_last_click_time = 0
        self.double_click_interval = 250  # 双击时间间隔（毫秒）

    def init_ui(self):
        self.setWindowTitle("奎氪绘图软件")
        # 窗口总宽度：原来的三列800 + 画布1000 = 1800
        # 现在去掉第三列，两列宽度约550 + 画布1000 = 1550
        self.setGeometry(50, 50, 1550, 900)
    ...
    def create_context_menu(self):
        ...
        # 创建菜单项
        self.menu_clear = QAction("清空画布", self)
        self.menu_save = QAction("保存绘图", self)
        self.menu_export = QAction("输出按钮位置", self)
        self.menu_separator = QAction(self)
        self.menu_separator.setSeparator(True)
        self.menu_exit = QAction("退出软件", self)

        # 设置快捷键（可选）
        self.menu_clear.setShortcut("Ctrl+L")
        self.menu_save.setShortcut("Ctrl+S")
        self.menu_export.setShortcut("Ctrl+E")
        self.menu_exit.setShortcut("Ctrl+Q")

        # 连接信号
        self.menu_clear.triggered.connect(self.clear_canvas)
        self.menu_save.triggered.connect(self.save_drawing)
        self.menu_export.triggered.connect(self.export_button_positions)
        self.menu_exit.triggered.connect(self.close)

        # 添加到菜单
        self.context_menu.addAction(self.menu_clear)
        self.context_menu.addAction(self.menu_save)
        self.context_menu.addAction(self.menu_export)
        self.context_menu.addAction(self.menu_separator)
        self.context_menu.addAction(self.menu_exit)
    ...
    def create_toolbar(self):
        toolbar = QFrame()
        # 工具栏总宽度：与原始代码的第一列+第二列宽度相同
        toolbar.setFixedWidth(550)  # 原来是三列800，去掉第三列后约550
        ...
        width_input_layout.addWidget(QLabel("输入粗细:"))

        # 创建输入框
        self.width_input = QLineEdit()
        self.width_input.setPlaceholderText("输入1-100之间的数字")
        self.width_input.setFixedWidth(80)
        self.width_input.returnPressed.connect(self.on_width_input_changed)
        ...
        width_container.addLayout(width_input_layout)
        ...
        width_container.addLayout(width_layout)
        left_column.addLayout(width_container)
        ...
        # === 第二列：整合原来的第二列和第三列内容 ===
        # 完全按照原始代码的布局和间距
        right_column.setContentsMargins(0, 0, 0, 0)  # 原始代码没有设置特殊边距
        ...
        # 第一行按钮 - 完全按照原始代码
        self.clear_btn = QPushButton("清空画布 (双击生效)")
        self.clear_btn.clicked.connect(self.on_clear_btn_clicked)  # 修改为双击处理
    ...
    def on_clear_btn_clicked(self):
        ...
        try:
            ...
            # 检查是否为双击
            is_double_click = (current_time - self.clear_last_click_time) < self.double_click_interval
            self.clear_last_click_time = current_time
            ...
            print(f"清空画布按钮点击错误: {e}")

    def on_width_input_changed(self):
        """处理线条粗细输入框的变化"""
        try:
            # 获取输入值
    ...
    def clear_canvas(self):
        ...
        if self.canvas:
            ...
            if reply == QMessageBox.Yes:
                ...
                QMessageBox.information(self, "提示", "画布已清空！")
